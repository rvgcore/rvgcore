...
#define GOSSIP_ITEM_DRUID_SIGNAL "Research Gossip"
...
#define GOSSIP_ITEM_BARK "Research Gossip"
...
public:
	...
	bool OnGossipHello(Player* player, GameObject* go)
	{
		...
		{
		case 801054:
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,"Research Gossip",GOSSIP_SENDER_CHECK,GOSSIP_ACTION_INFO_DEF+1);
			break;
		case 801055:
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,"Research Gossip",GOSSIP_SENDER_CHECK,GOSSIP_ACTION_INFO_DEF+2);
			break;
		}
...
#define GOSSIP_ITEM_TABARD    "Research Gossip"
#define GOSSIP_ITEM_REQUEST_A "Research Gossip"
#define GOSSIP_ITEM_REQUEST_H "Research Gossip"
...
enum SimonEvents
{
	...
	EVENT_SIMON_CORRECT_FULL_SEQUENCE,
	EVENT_SIMON_WRONG_SEQUENCE,
	...
	struct npc_simon_bunnyAI : public ScriptedAI
	{
		...
		void UpdateAI(const uint32 diff)
		{
			...
			case EVENT_SIMON_RESET_CLUSTERS:
				...
				break;
			case EVENT_SIMON_CORRECT_FULL_SEQUENCE:
				gameLevel++;
				events.ScheduleEvent(EVENT_SIMON_ROUND_FINISHED, 0);
				break;
			case EVENT_SIMON_TOO_LONG_TIME:
			case EVENT_SIMON_WRONG_SEQUENCE:
				GivePunishment();
				events.ScheduleEvent(EVENT_SIMON_ROUND_FINISHED, 0);
				break;
		...
		/*
        Called by color clusters script and used for knowing the button pressed by player
		 */
		 void SetData(uint32 type, uint32 /*data*/)
		{
        	uint8 pressedColor;

        	if (type == clusterIds[SIMON_RED])         pressedColor = SIMON_RED;
        	else if (type == clusterIds[SIMON_BLUE])   pressedColor = SIMON_BLUE;
        	else if (type == clusterIds[SIMON_GREEN])  pressedColor = SIMON_GREEN;
        	else if (type == clusterIds[SIMON_YELLOW]) pressedColor = SIMON_YELLOW;
		 ...
		 */
		 void StartGame()
		 {
			 ...
			 Trinity::AllWorldObjectsInRange objects(me, 5.0f);
			 ...
			 if(GameObject* relic = me->FindNearestGameObject(GO_APEXIS_RELIC, 5.0f))
        ...
        Called when despawning the bunny. Sets all the node GOs to their default states.
		 ...
		 void ResetNode()
		 {
			 ...
			 for (uint32 clusterId = SIMON_BLUE; clusterId < SIMON_MAX_COLORS; clusterId++)
				 if(GameObject* cluster = me->FindNearestGameObject(clusterIds[clusterId], 5.0f))
					 cluster->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);

			 for (uint32 auraId = GO_AURA_BLUE; auraId <= GO_AURA_YELLOW; auraId++)
				 if(GameObject* auraGo = me->FindNearestGameObject(auraId, 5.0f))
        ...
        round, if not, give punishment and restart current level.
		 ...
		 void CheckPlayerSequence()
		 {
			 ...
			 if (correct && (playerSequence.size() == colorSequence.size()))
				 events.ScheduleEvent(EVENT_SIMON_CORRECT_FULL_SEQUENCE, 0);
			 else if (!correct)
				 events.ScheduleEvent(EVENT_SIMON_WRONG_SEQUENCE, 0);
		 }
		 ...
		 void SetUpPreGame()
		 {
			 for (uint32 clusterId = SIMON_BLUE; clusterId < SIMON_MAX_COLORS; clusterId++)
			 {
				 if(GameObject* cluster = me->FindNearestGameObject(clusterIds[clusterId], 5.0f))
				 {
					 ...
					 if (cluster->GetEntry() == clusterIds[SIMON_RED])         preGameSpellId = SPELL_PRE_GAME_RED;
					 else if (cluster->GetEntry() == clusterIds[SIMON_BLUE])   preGameSpellId = SPELL_PRE_GAME_BLUE;
					 else if (cluster->GetEntry() == clusterIds[SIMON_GREEN])  preGameSpellId = SPELL_PRE_GAME_GREEN;
					 else if (cluster->GetEntry() == clusterIds[SIMON_YELLOW]) preGameSpellId = SPELL_PRE_GAME_YELLOW;
			 ...
			 if (GameObject* relic = me->FindNearestGameObject(GO_APEXIS_RELIC, 5.0f))
			 {
				 ...
				 me->NearTeleportTo(x, y, z + 2.75f, o);
			 }
        ...
        Handles the spell rewards. The spells also have the QuestCompleteEffect, so quests credits are working.
		 ...
		 void GiveRewardForLevel(uint8 level)
		 {
			 ...
			 switch (level)
			 {
			 case 6:  rewSpell = SPELL_REWARD_BUFF_1; break;
			 case 8:  rewSpell = SPELL_REWARD_BUFF_2; break;
			 case 10: rewSpell = SPELL_REWARD_BUFF_3; break;
			 }
        ...
        4 fails = death.
		 ...
		 void GivePunishment()
		 {
			 ...
			 if (Player* player = me->GetPlayer(*me, playerGUID))
			 {
				 int32 bp = (int32)((float)(fails)*0.33f*player->GetMaxHealth());
				 player->CastCustomSpell(player, SPELL_BAD_PRESS_DAMAGE, &bp, NULL, NULL, true);
...
public:
	...
	bool OnGossipHello(Player* player, GameObject* go)
	{
		if (Creature* bunny = go->FindNearestCreature(NPC_SIMON_BUNNY, 5.0f, true))
...
enum ApexisRelic
{
	...
	SPELL_TAKE_REAGEANTS_SOLO  = 41145,
	SPELL_TAKE_REAGEANTS_GROUP = 41146,
...
public:
	...
	bool OnGossipHello(Player* player, GameObject* go)
	{
		if (player->GetQuestStatus(QUEST_CRYSTALS) == QUEST_STATUS_REWARDED)
			player->ADD_GOSSIP_ITEM(0, "research gossip", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
	...
	bool OnGossipSelect(Player* player, GameObject* go, uint32 /*sender*/, uint32 /*action*/)
	{
		...
		if (player->GetGroup() && player->HasItemCount(ITEM_APEXIS_SHARD, 35))
			player->CastSpell(player, SPELL_TAKE_REAGEANTS_GROUP, false);
		else if (player->HasItemCount(ITEM_APEXIS_SHARD, 1))
			player->CastSpell(player, 41145, false);
		else
			return true;

		if (Creature* bunny = player->SummonCreature(NPC_SIMON_BUNNY, go->GetPositionX(), go->GetPositionY(), go->GetPositionZ()))
			bunny->AI()->SetGUID(player->GetGUID(), 0);
		return true;
	}
};

class spell_shroud_of_the_scourge_areacheck : public SpellScriptLoader
{
public:
	spell_shroud_of_the_scourge_areacheck() : SpellScriptLoader("spell_shroud_of_the_scourge_areacheck") { }

	class spell_shroud_of_the_scourge_areacheck_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_shroud_of_the_scourge_areacheck_AuraScript);

		bool CheckAreaTarget(Unit* target)
		{
			if (target->GetAreaId() == 4125)
				return true;
			return false;
		}
		...
		void Register()
		{
			DoCheckAreaTarget += AuraCheckAreaTargetFn(spell_shroud_of_the_scourge_areacheck_AuraScript::CheckAreaTarget);
		}
	};

	AuraScript* GetAuraScript() const
	{
		return new spell_shroud_of_the_scourge_areacheck_AuraScript();
	}
};
...
void AddSC_custom_spell_scripts()
{
	...
	new go_apexis_relic();
	new spell_shroud_of_the_scourge_areacheck();
}
